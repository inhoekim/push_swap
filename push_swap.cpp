	...
	if (from == A) {
		for (int i = 0; i < size; i++) {
			...
			if (dqs[A].front() < pivot) {
				...
				PP(A, B);
				//큰영역(L)
			} else if (dqs[A].front() >= pivot2) {
				RO(A);
			}
		}
		//RRO를 해서 파티셔닝한 L을 위로 올리는게 나은지, RO를 해서 기존 덱영역을 밑으로 내리는게 나은지 확인(최적화 과정)
		if (dqs[A].size() - ps[2] > ps[2])
			for (int j = 0; j < ps[2]; j++)
				RRO(A);
		else
			for (int j = 0; j < dqs[A].size() - ps[2]; j++)
				RO(A);
	}
		//B에서 파티션이 일어난 경우
	else {
		for (int i = 0; i < size; i++) {
			if (dqs[B].front() < pivot) {
				RO(B);
		...
		if (dqs[B].size() - ps[0] > ps[0])
			for (int j = 0; j < ps[0]; j++)
				RRO(B);
		else
			for (int j = 0; j < dqs[B].size() - ps[2]; j++)
				RO(B);
	}
}
